...
export const FeaturesBlock: React.FC<FeatureBlockProps> = ({ title, features }) => {
  return (
    <div className="px-8 md:px-20 xl:px-28 mx-auto container">
      ...
      {features && (
        <div className="flex max-lg:flex-col items-center justify-center gap-4 xl:gap-16">
          {features.map((feature) => (
            <div
              className="flex-1 max-lg:w-full max-w-[24rem] p-10 rounded-xl border border-primary/30 flex flex-col items-center justify-center bg-muted"
              key={feature.id}
            >
